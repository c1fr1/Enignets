	...
	public Shader(String path, int stype) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				if (line.startsWith("#")) {
					if (line.equals("#import <gnoise>")) {
						BufferedReader br = new BufferedReader(new InputStreamReader(getClass().getClassLoader().getResourceAsStream("engine/noise.glsl")/*path*/));
					...
					if (line.startsWith("#import \"")) {
						BufferedReader br = new BufferedReader(new InputStreamReader(getClass().getClassLoader().getResourceAsStream(line.substring(9, line.length() - 1))/*path*/));
						String brLine = br.readLine();
						line = "";
						while (brLine != null) {
							line += brLine + "\n";
